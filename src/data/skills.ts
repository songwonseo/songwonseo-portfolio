...
const levelOf = (name: string): SkillLevel => (Object.keys(levels) as SkillLevel[]).find((level) => levels[level].includes(name)) ?? 'BASIC'
const group = (title: string, names: string[]): SkillGroup => ({ title, items: names.map((name) => ({ name, level: levelOf(name) })) })
