...
export function ProjectPage(){
  ...
  return <main className={`project-page${project.accent?' lifequest-theme':''}`}>
    <section className="project-hero"><div className="container"><Link className="back-link" to="/#projects"><ArrowLeft/> 프로젝트 목록</Link><div className="project-hero-grid"><div><span className="project-eyebrow">CASE STUDY · 0{index+1}</span><h1>{project.displayTitle}</h1><p>{project.subtitle}</p><span className={`status status--${project.status.replace(' ','-').toLowerCase()}`}>{project.status}</span></div><ImageWithFallback src={project.thumbnail} alt={`${project.title} 프로젝트 대표 이미지`} label={project.title}/></div></div></section>
