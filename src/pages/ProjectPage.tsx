...
export function ProjectPage() {
  ...
  return (
    <main className={`project-page project-theme--${project.id}`}>
      ...
      <section className="case-section project-introduction">
        <div className="container split-section">
          <div>
            <span className="case-label">INTRODUCTION</span>
            <h2>프로젝트 소개</h2>
          </div>
      ...
      <section className="case-section">
        <div className="container split-section">
          ...
          <ul className="role-list">
            {project.responsibilities.map((x, index) => (
              <li key={x}>
                <span>0{index + 1}</span>
                {x}
              </li>
      ...
      <section className="case-section flow-section">
        <div className="container">
          <div className="case-heading">
            <span className="case-label">PROCESS</span>
            <h2>구현 흐름</h2>
          </div>
          <div className="flow">
            {project.flow.map((x, index) => (
              <div key={x.title}>
          ...
          {project.adminFlow && (
            <div className="admin-flow">
              <h3>관리자 흐름</h3>
              <div className="flow">
                {project.adminFlow.map((x, index) => (
                  <div key={x.title}>
                    <span>{String(index + 1).padStart(2, "0")}</span>
                    <strong>{x.title}</strong>
                    <p>{x.description}</p>
                    {index < project.adminFlow!.length - 1 && (
                      <ArrowRight aria-hidden="true" />
                    )}
                  </div>
                ))}
      ...
      <section className="case-section">
        <div className="container">
          <div className="case-heading">
            <span className="case-label">SCREENS</span>
            <h2>주요 화면</h2>
          </div>
          <div
              ...
              <figure className={s.type === "desktop" ? "screen--desktop" : undefined} key={s.title}>
                {s.src ? (
                  ...
                  <div
                    ...
                    role="img"
                    aria-label={`${s.title} 앱 목업 이미지 준비 중`}
                  >
                    <div className="phone-speaker" />
                    <span>COMING SOON</span>
                    <strong>APP MOCKUP</strong>
      ...
      <section className="case-section troubleshooting">
        <div className="container">
          <div className="case-heading">
            <span className="case-label">TROUBLESHOOTING</span>
            <h2>문제 해결 기록</h2>
          </div>
          <div className="trouble-grid">
            {project.troubles.map((item) => (
      ...
      <section className="case-section reflection">
        <div className="container">
          <span className="case-label">RETROSPECTIVE</span>
          <h2>배운 점</h2>
